...
impl InternalLogger {
    pub async fn init(
        ...
        log_cancel_token: CancellationToken,
    ) -> Option<Arc<Self>> {
        ...
        task::spawn(log.clone().log_loop(receiver, log_cancel_token.clone()));
    ...
    async fn log_loop(
        self: Arc<Self>,
        mut receiver: mpsc::UnboundedReceiver<(Level, LogData)>,
...
impl SteelLogger for InternalLogger {
...
pub struct CommandLogger(Arc<InternalLogger>);
impl CommandLogger {
    ...
    pub async fn new(
        ...
        Some(Self(
            InternalLogger::init(history_path, cancel_token, log_cancel_token).await?,
        ))
    }
}

impl<S: Subscriber> Layer<S> for CommandLogger {

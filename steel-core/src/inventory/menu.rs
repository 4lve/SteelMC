...
use crate::{
    ...
    player::{Player, connection::PlayerConnection},
};
use std::sync::Arc;
...
impl MenuBehavior {
    ...
    fn send_packet<P: ClientPacket>(connection: &Arc<dyn PlayerConnection>, packet: P) {
        let encoded =
    ...
    pub fn send_all_data_to_remote(&mut self, connection: &Arc<dyn PlayerConnection>) {
    ...
    pub fn broadcast_changes(&mut self, connection: &Arc<dyn PlayerConnection>) {
        let guard = self.lock_all_containers();
    ...
    fn synchronize_slot_to_remote(&mut self, slot: usize, connection: &Arc<dyn PlayerConnection>) {
    ...
    fn synchronize_carried_to_remote(&mut self, connection: &Arc<dyn PlayerConnection>) {
    ...
    pub fn broadcast_full_state(&mut self, connection: &Arc<dyn PlayerConnection>) {
